...
Aangezien de ingangen van de onderliggende schakeling rechtstreeks verbonden zijn met twee registers, is het noodzakelijk om voor elke bewerking de nodige waarden naar die registers over te brengen. Gezien het feit dat er echter vijftien registers zijn (tegenover zes in \cite{lee}), zou het veel energie kosten om per 'shift' operatie aan elk van de vijftien registers nieuwe waarden toe te wijzen. Verder zou het gemiddeld veel klokslagen in beslag nemen om beide waarden in de correcte registers op te slaan. Grofweg kan gesteld worden dat de benodigde waarden $x_0$ en $x_1$ gemiddeld $\frac{n}{2}$ registers van elkaar verwijderd zullen zijn. Er zijn dan $\frac{n}{2}$ `swap' operaties nodig om de registers naast elkaar te krijgen. Verder zijn er per `swap' operatie $n$ `shift' operaties nodig voor $x_0$ opnieuw een positie dichter bij $x_1$ kan gebracht worden. Na het be\"eindigen van deze operatie zijn er dus grof geschat $\frac{n}{2} \cdot n$ maal $n$ registers van waarde veranderd. De gehele operatie vraagt dus een tijdscomplexiteit van $\mathcal{O}(n^2)$ en er dienen $\mathcal{O}(n^3)$ schrijfbewerkingen uitgevoerd te worden.

 Om het energieverbruik enigszins te matigen, wordt het mogelijk gemaakt om elk register ook de waarde van zijn voorganger aan te laten nemen. Dit is equivalent aan de `swap' (en `copy') operatie toe te laten tussen alle aan elkaar grenzende registers. `swap' operaties tussen twee paar registers kunnen in parallel uitgevoerd en elke `swap' operatie vraagt twee schrijfbewerkingen. Er zullen dus gemiddeld $2 \cdot \frac{n}{2}$ schrijfbewerkingen uitgevoerd worden per benodigde waarde, $2n$ in totaal dus. Met dit aaangepast ontwerp komt dit alles dus neer op een tijds- en schrijfbewerkingscomplexiteit $\mathcal{O}(n)$, respectievelijk een factor $n$ en $n^2$ lager. Uiteraard wordt hiervoor een prijs betaald, want er zal nu per register een extra multiplexer voorzien moeten worden. Het geoptimaliseerde ontwerp is te zien in \reffig{figuur-implementatie-miller-geheugen-circ-optimized}.
