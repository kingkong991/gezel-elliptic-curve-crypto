\tikzset {
	...
	powercirc/.style={circ, power, xshift=0.4mm}
}
...
% Connect not gates
\draw [bit] (n3.output) -- (n4.input)
		(n5.output) -| ($(n2.input) - (3mm, 0)$ -| n5.output) node[joint, minimum height=0.15cm] {}
...
\draw [line width=1.7pt] (i1.base) -- node[input] {D} (n1.input)
		(n1.output) -- (n2.input)
		(n2.output) -- (n3.input)
		(n5.input) -| ($(n2.output) + (3mm, 0)$ -| n5.input) node[joint, minimum height=0.15cm] {}
...
\draw	[bit, line cap=rect] ($(n3.north)!0.5!(n3.north east)$) -- ++(0, 3mm) node[ct] {CN}
...
% Same, but in bold
\draw	[line width=1.7pt, line cap=rect] ($(n1.north)!0.5!(n1.north east)$) -- ++(0, 3mm) node[ct] {CP}
		($(n1.south)!0.5!(n1.south east)$) -- ++(0, -4mm) node[cb] {CN};


% Add weird stuff
\draw (n1.south) node[powercirc] {}
		(n3.south) node[circ] {}
