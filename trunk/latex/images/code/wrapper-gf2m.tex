...
%	\node[logic, xshift=-15mm] (l3) {};
	...
	\node[logic, xshift=-9mm, yshift=6mm] (l1) {};
...
\draw		[multibit] (j1.base) -| ($(m2.south east) + (-1mm, 0)$)
			...
			(a1.east) -- node [above, near start, yshift=-0.7mm, xshift=1mm] {B$_{\text{in}}$} (mcore.west);
...
\draw		[bit] %(m1.north) |- ($(a1.north west) + (0, -1.5mm)$)
			...
			(m1.west) -- node[xshift=-1.7mm] {s$_1$} ($(m1.west) - (0, 3mm)$);
